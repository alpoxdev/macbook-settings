...
function writeStatusDiagnostic(event: string, outcome: string, status?: number): void {
  const paneKey = process.env.ORCA_PANE_KEY
  const safePaneKey = paneKey?.replace(/[^a-zA-Z0-9_-]/g, '_')
  const path =
    process.env.ORCA_PI_STATUS_DIAGNOSTIC ??
    (process.env.HOME && safePaneKey
      ? `${process.env.HOME}/.omo/agent/orca-pi-adapter/status/${safePaneKey}.json`
      : undefined)
  if (!path) return
  try {
    const fs = getNodeBuiltin('fs')
    const pathModule = getNodeBuiltin('path')
    fs?.mkdirSync(pathModule.dirname(path), { recursive: true, mode: 0o700 })
    fs?.writeFileSync(
      path,
      `${JSON.stringify({ event, outcome, ...(status === undefined ? {} : { status }), at: new Date().toISOString() })}\n`,
      { encoding: 'utf8', mode: 0o600 }
    )
  } catch {
    // Diagnostics are optional and must never affect the agent.
  }
}
...
function isAncestorProcess(ownerPid: string): boolean {
  ...
  try {
    ...
    while (current > 1 && !seen.has(current)) {
      ...
      seen.add(current)
      const output = childProcess?.execFileSync('/bin/ps', ['-o', 'ppid=', '-p', String(current)], {
        encoding: 'utf8'
      })
      current = Number.parseInt(String(output).trim(), 10)
...
async function postOnce(
  ...
  if (!coords.port || !coords.token || !paneKey) {
    writeStatusDiagnostic(hookEventName, 'missing-coordinates')
    return
  }
  ...
  try {
    ...
    if (!response.ok) {
      writeStatusDiagnostic(hookEventName, 'rejected', response.status)
      return
    }
    writeStatusDiagnostic(hookEventName, 'accepted', response.status)
  } catch {
    writeStatusDiagnostic(hookEventName, 'transport-error')
...
function postViaWindowsCurl(url: string, coords: { token: string }, body: string): void {
  ...
  if (!curlPath) return
  try {
    const { spawn } = getNodeBuiltin('child_process')
    const child = spawn(
      curlPath,
      [
        '-sS',
...
export default function (pi): void {
  const extensionKey = Symbol.for('orca-pi-status-extension')
  if (globalThis[extensionKey]) return
  const ownerPid = process.env.ORCA_PI_STATUS_OWNED
  const selfPid = String(process.pid)
  if (ownerPid && ownerPid !== selfPid && isAncestorProcess(ownerPid)) return
